...
describe("content-pack parser", () => {
  ...
  it("rejects corrupt files that only use an image extension", () => {
    expect(() =>
      parseContentPack(pack({}, Buffer.from("not an image"))),
    ).toThrow(/valid PNG, JPEG, or WEBP/i);
  });
  ...
  it("rejects duplicate post references", () => {
    ...
    expect(() => parseContentPack(zip.toBuffer())).toThrow(
      /Duplicate post reference/i,
    );
  });

  it("accepts Instagram-only Story entries", () => {
  ...
  it("rejects Stories that target a feed platform", () => {
    expect(() =>
      parseContentPack(
        pack({ format: "story", platforms: ["facebook", "instagram"] }),
      ),
    ).toThrow(/Story must target Instagram only/i);
  });
});
