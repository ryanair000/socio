...
if (!fs.existsSync(manifestPath))
  throw new Error("manifest.json is missing from the source directory.");
...
for (const [index, post] of manifest.posts.entries()) {
  if (!post.reference || refs.has(post.reference))
    throw new Error(`Post ${index + 1} has a missing or duplicate reference.`);
  refs.add(post.reference);
  if (!post.title || post.title.length > 120)
    throw new Error(
      `${post.reference} needs a title of at most 120 characters.`,
    );
  if (String(post.caption ?? "").length > 2200)
    throw new Error(`${post.reference} caption exceeds 2,200 characters.`);
  if (
    !Array.isArray(post.media) ||
    post.media.length < 1 ||
    post.media.length > 10
  )
    throw new Error(`${post.reference} needs 1-10 media files.`);
  if (!["single", "carousel", "story", undefined].includes(post.format))
    throw new Error(`${post.reference} has an unsupported format.`);
  if (
    (post.format === "carousel" || post.media.length > 1) &&
    post.media.length < 2
  )
    throw new Error(`${post.reference} carousel needs at least two slides.`);
  if (post.format === "story" && post.media.length !== 1)
    throw new Error(
      `${post.reference} Instagram Story needs exactly one image.`,
    );
  ...
  if (
    ...
    throw new Error(
      `${post.reference} Instagram Story must target Instagram only.`,
    );
  for (const rawName of post.media) {
    const name = safeRelative(rawName);
    const absolute = path.resolve(source, name);
    if (
      !absolute.startsWith(`${source}${path.sep}`) ||
      !fs.existsSync(absolute) ||
      !fs.statSync(absolute).isFile()
    )
      throw new Error(`${post.reference} is missing ${name}.`);
    const bytes = fs.readFileSync(absolute);
    if (bytes.length > 12 * 1024 * 1024)
      throw new Error(`${name} exceeds 12 MB.`);
    imageType(bytes, name);
    media.set(name, bytes);
  }
}
if (media.size > 70)
  throw new Error("A pack cannot contain more than 70 unique images.");
const expanded =
  fs.statSync(manifestPath).size +
  [...media.values()].reduce((sum, value) => sum + value.length, 0);
