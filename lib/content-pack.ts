...
import type { Brand, PostFormat, PublishPlatform, QaStatus } from "@/lib/types";
...
function contentTypeFromSignature(
  filename: string,
  bytes: Buffer,
): ParsedAsset["contentType"] {
  if (
    bytes.length >= 8 &&
    bytes
      .subarray(0, 8)
...
function normalizePlatforms(value: unknown, fallback: PublishPlatform[]) {
  const supported = new Set<PublishPlatform>([
    "facebook",
    "instagram",
    "tiktok",
  ]);
    ...
    ...new Set(
      values.filter(
        (item): item is PublishPlatform =>
          typeof item === "string" && supported.has(item as PublishPlatform),
      ),
    ),
  ];
  if (!platforms.length)
    throw new Error("Every post needs at least one supported platform.");
  return platforms;
}
...
function normalizePolicy(value: unknown): OverduePolicy {
  return value === "keep_draft" ||
    value === "skip_expired" ||
    value === "stagger_from_now"
    ? value
    : "roll_forward";
}
...
function scheduleDate(value: RawManifestPost["schedule"], timezone: string) {
  ...
  if (Number.isNaN(parsed.getTime()))
    throw new Error("A scheduled date is invalid.");
  return parsed;
}
...
function parseManifestPack(
  ...
  const timezone =
    cleanString(raw.timezone, "Timezone", 64) || NAIROBI_TIME_ZONE;
  const name =
    cleanString(raw.campaign?.name, "Campaign name", 160) ||
    "Imported content pack";
  const sourceWeek =
    cleanString(raw.campaign?.sourceWeek, "Source week", 64) || null;
  const fallbackPlatforms = normalizePlatforms(raw.defaults?.platforms, [
    "facebook",
    "instagram",
  ]);
  ...
  if (assetEntries.length > MAX_PACK_IMAGES)
    throw new Error("A content pack cannot contain more than 70 images.");
  ...
  for (const asset of assets)
    duplicateHashes.set(asset.sha256, [
      ...(duplicateHashes.get(asset.sha256) ?? []),
      asset.filename,
    ]);
  for (const filenames of duplicateHashes.values()) {
    if (filenames.length > 1) {
      checks.push({
        ...
        severity: "warning",
        message: `Identical image bytes appear in: ${filenames.join(", ")}.`,
      });
    }
  }

  const seenRefs = new Set<string>();
  const normalizedEntries: ParsedEntry[] = (raw.posts as RawManifestPost[]).map(
    (post, index) => {
      const reference = cleanString(
        post.reference,
        `Post ${index + 1} reference`,
        120,
        true,
      );
      ...
      if (
        ...
        throw new Error(
          `${reference} must contain between 1 and 10 media files.`,
        );
      }
      const media = post.media.map((value) =>
        normalizedArchivePath(
          cleanString(value, `${reference} media`, 240, true),
        ),
      );
      for (const filename of media) {
        const exact = byPath.get(filename.toLowerCase());
        const basenameMatches =
          byBasename.get(filename.split("/").pop()!.toLowerCase()) ?? [];
        if (!exact && basenameMatches.length !== 1)
      ...
      if (format === "carousel" && media.length < 2)
        throw new Error(`${reference} carousel needs at least two slides.`);
      if ((format === "single" || format === "story") && media.length !== 1) {
        throw new Error(
          format === "story"
            ? `${reference} Instagram Story must have one image.`
            : `${reference} independent post must have one image.`,
        );
      }
      const qaStatus = normalizeQa(post.qaStatus);
      const holdReason =
        cleanString(post.holdReason, `${reference} hold reason`, 500) || null;
      if (qaStatus === "hold")
        checks.push({
          entryRef: reference,
          code: "qa_hold",
          severity: "error",
          message: holdReason || "This post is on QA hold.",
        });
      if (qaStatus === "ready_after_qa")
        checks.push({
          entryRef: reference,
          code: "qa_confirmation_required",
          severity: "warning",
          message: holdReason || "Confirm artwork QA before approval.",
        });
      const detectedText = cleanString(
        post.detectedText,
        `${reference} detected text`,
        5000,
      );
      if (brand === "chezahub")
        checks.push(
          ...phoneChecks(reference, `${title}\n${caption}\n${detectedText}`),
        );
      ...
      const contentHash = hash(
        JSON.stringify({
          ...
          media: media.map(
            (name) =>
              (
                byPath.get(name.toLowerCase()) ??
                byBasename.get(name.split("/").pop()!.toLowerCase())?.[0]
              )?.sha256,
          ),
          platforms,
        }),
...
function parseLegacyPack(buffer: Buffer): ParsedContentPack {
  ...
  const entries = posts.map((post) => {
    if (post.qaStatus === "hold")
      checks.push({
        entryRef: post.sourceRef,
        code: "qa_hold",
        severity: "error",
        message: post.holdReason || "This post is on QA hold.",
      });
    if (post.qaStatus === "ready_after_qa")
      checks.push({
        entryRef: post.sourceRef,
        code: "qa_confirmation_required",
        severity: "warning",
        message: post.holdReason || "Confirm artwork QA before approval.",
      });
    checks.push(
      ...phoneChecks(post.sourceRef, `${post.title}\n${post.caption}`),
    );
    return {
      ...
      contentHash: hash(
        JSON.stringify({
          title: post.title,
          caption: post.caption,
          media: post.media.map((item) => hash(item.bytes)),
        }),
      ),
    };
  });
...
export function parseContentPack(buffer: Buffer): ParsedContentPack {
  if (buffer.length > MAX_PACK_BYTES)
    throw new Error("Content packs cannot exceed 25 MB.");
  const archive = new AdmZip(buffer);
  const entries = archive.getEntries().filter((entry) => !entry.isDirectory);
  for (const entry of entries) normalizedArchivePath(entry.entryName);
  const manifest = entries.find((entry) =>
    entry.entryName.replace(/\\/g, "/").toLowerCase().endsWith("manifest.json"),
  );
  return manifest
    ? parseManifestPack(buffer, archive, manifest)
    : parseLegacyPack(buffer);
}
